...
void CWhisperAdmin::SaveLog(LPCHARACTER ch, const char* c_pszText, const char* c_pszLang, int color)
{
	...
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO log.log_whisper_admin (who, date, text, lang, color) VALUES('%s', NOW(), '%s', '%s', '%d')", ch->GetName(), c_pszText, c_pszLang, color);
...
void CWhisperAdmin::Manager(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[SYSTEM] Textul este prea lung.");
		return;
	}
