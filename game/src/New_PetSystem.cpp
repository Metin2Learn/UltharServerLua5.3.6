...
bool CNewPetActor::IncreasePetSkill(int skill) {
	...
	for (int i = 0; i < 3; ++i) {
		if (m_dwskillslot[i] == skill) {
			if (m_dwskill[i] < 20) {
				m_dwskill[i] += 1;
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_beceri_ulasti"), m_dwskill[i]);
				m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
				return true;
			}
			else {
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_beceri_max"));
				return false;
			}
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		if (m_dwskillslot[i] == 0) {
			...
			m_dwskill[i] = 1;
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_beceri_yeni"));
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
			return true;
		}
	}
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_beceri_yeni_yok"));
	return false;
}
...
bool CNewPetActor::IncreasePetEvolution() {
	if (m_dwevolution < 3) {
		...
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_evrim_yok"));
		return false;
	}
	return true;
}
...
void CNewPetActor::Unsummon()
{
	if (true == this->IsSummoned())
	{
		...
		#ifdef ENABLE_NEW_PET_IMMORTAL_FIX
...
DWORD CNewPetActor::Summon(const char* petName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	pSummonItem->SetForceAttribute(6, 1, GetSkillCount());
	pSummonItem->SetSocket(1,m_dwlevel);
	pSummonItem->SetSocket(2, 1, m_dwPetType);
...
void CNewPetActor::DoPetSkill(int skillslot) {
	...
	case 10:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(0) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_yetenek_bekle"), 480 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(0)));
			return;
		}
		if (m_pkOwner->GetHPPct() > 20) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_yetenek_bu"));
			return;
		}
	...
	case 17:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(1) <= 600) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_yetenek_yeniden"), 600 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(1)));
			return;
		}
		m_pkOwner->SetNewPetSkillCD(1, get_global_time());
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_olumsuz"));
	...
	case 18:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(2) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("pet_yetenek_yeniden"), 480 -(get_global_time() - m_pkOwner->GetNewPetSkillCD(2)));
			return;
		}
