...
void CPetActor::SetName(const char* name)
{
	...
	if (true == IsSummoned())
	{
		DWORD lv = m_pkOwner->GetQuestFlag("pet_system.pet_level");
		if (lv < 1)
			lv = 1;
		
		char result[128];
		snprintf(result, sizeof(result), "|cFFFFFF00Lv. %d|r %s", lv, petName.c_str());

		m_pkChar->SetName(result);
	}
...
void CPetActor::Unsummon()
{
	if (true == this->IsSummoned())
	{
		...
		this->ClearBuff();
#ifdef INTERN_PET_MOD
        LPITEM smItem = ITEM_MANAGER::instance().FindByVID(m_dwSummonItemVID);
        if(NULL != smItem){
            smItem->SetSocket(1,false);
            smItem->Lock(false);
        }
#endif
