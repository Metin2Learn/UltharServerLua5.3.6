...
EVENTFUNC(oxevent_timer)
{
	...
	switch (flag)
	{
		...
		case 1:
			...
			SendNoticeMap(LC_TEXT("Á¤´äAo"), OXEVENT_MAP_INDEX, true);
			...
			if (info->answer == true)
			{
				...
				SendNoticeMap(LC_TEXT("O AÔ´I´U"), OXEVENT_MAP_INDEX, true);
			...
			else
			{
				...
				SendNoticeMap(LC_TEXT("X AÔ´I´U"), OXEVENT_MAP_INDEX, true);

			}

			if (LC_IsJapan())
			{
				SendNoticeMap("ŠÔˆá‚¦‚½•ûX‚ðŠO‚ÉˆÚ“®‚³‚¹‚Ü‚·B", OXEVENT_MAP_INDEX, true);
			}
			else
			{

				SendNoticeMap(LC_TEXT("5AE µÚ A2¸®1A o?µéA» 1U±uA¸·Î AIµ? 1AA°°Ú1A´I´U."), OXEVENT_MAP_INDEX, true);
		...
		case 2:
			...
			SendNoticeMap(LC_TEXT( "´UA1 1®Á¦ ÁOonÇOÁÖ11?ä."), OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap(LC_TEXT("1®Á¦ AÔ´I´U."), OXEVENT_MAP_INDEX, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true);
	SendNoticeMap(LC_TEXT("¸ÂA¸¸é O, A2¸®¸é X·Î AIµ?ÇOÁÖ11?ä"), OXEVENT_MAP_INDEX, true);
