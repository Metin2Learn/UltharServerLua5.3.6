...
void CInputDB::ShopUpdateItem(const char * c_pData)
{
	...
	if (CHARACTER_MANAGER::instance().GetCharactersByRaceNum(30000, i))
	{
		...
		while (it != i.end()) {
			LPCHARACTER pc = *it++;
			if (pc)
				if (pc->GetPrivShop() == p->shop_id && pc->GetMyShop()) {
					pc->SetShopEditModeTick();
					if(p->refresh)
						pc->UpdateShopItems();
					else if(p->shop_locked || !p->tick && !p->shop_locked)
						pc->SetShopEditMode(p->shop_locked);					
					return;
				}
...
void CInputDB::ReloadProto(const char * c_pData)
{
	...
	switch(LOWER(cType))
	{
		case 'k':
			...
			c_pData += sizeof(WORD);
			sys_log(0, "RELOAD: SKILL: %d", wSize);
			if (wSize)
		...
		case 'b':
			...
			wSize = decode_2bytes(c_pData);
			sys_log(0, "RELOAD: BANWORD: %d", wSize);
			c_pData += sizeof(WORD);
		...
		case 'l':
			 ...
			 */
			using namespace building;
			if (decode_2bytes(c_pData) != sizeof(TLand))
			{
				sys_err("land table size error");
				thecore_shutdown();
				return;
			...
			wSize = decode_2bytes(c_pData);
			sys_log(0, "RELOAD: LANDS: %d", wSize);
			c_pData += 2;

			if (wSize)
			{
				TLand * kLand = (TLand *) c_pData;
				c_pData += wSize * sizeof(TLand);

				for (WORD i = 0; i < wSize; ++i, ++kLand)
					CManager::instance().LoadLand(kLand);
			}
			break;
		...
		case 'o':
			 ...
			 */
			using namespace building;
			if (decode_2bytes(c_pData) != sizeof(TObjectProto))
			{
				sys_err("object proto table size error");
				thecore_shutdown();
				return;
			}
			sys_log(0, "RELOAD: OBJECT PROTO: %d", wSize);
			c_pData += 2;

			wSize = decode_2bytes(c_pData);
			c_pData += 2;

			CManager::instance().LoadObjectProto((TObjectProto *) c_pData, wSize);
			c_pData += wSize * sizeof(TObjectProto);
			break;
		...
		case 'n':
			 ...
			 */
			using namespace building;
			if (decode_2bytes(c_pData) != sizeof(TObject))
			{
				sys_err("object table size error");
				thecore_shutdown();
				return;
			}
			sys_log(0, "RELOAD: OBJECT: %d", wSize);
			c_pData += 2;

			wSize = decode_2bytes(c_pData);
			c_pData += 2;

			TObject * kObj = (TObject *) c_pData;
			c_pData += wSize * sizeof(TObject);

			for (WORD i = 0; i < wSize; ++i, ++kObj)
				CManager::instance().LoadObject(kObj, true);
			break;

		// default:
			// sys_log(0, "RELOAD - UNKNOWN TYPE: %c", cType);
			// break;
	}
	CMotionManager::instance().Build();
