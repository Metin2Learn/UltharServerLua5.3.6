...
const char* txt_gaya[] = {
	"[Gaya]No tiene suficiente gaya.",
	"[Gaya]Error 001.",
	"[Gaya]Necesita %d de %s.",
	"[Gaya]No tiene suficiente yang.",
	"[Gaya]Cantidad de Gaya: %d.",
	"[Gaya]El corte fue un exito.",
	"[Gaya]El corte falló."
};
...
EVENTFUNC(check_time_market_event)
{
	...
	{
		ch->SetGayaState("system_gaya.gaya_time_world_4", init_gayaTime() +  30);
...
void CHARACTER::StartCheckTimeMarket()
{
	...
	GayaUpdateTime = event_create(check_time_market_event, info, PASSES_PER_SEC(5));	// 1ºÐ
}