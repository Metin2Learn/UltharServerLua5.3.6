	...
	if (prob <= prt->prob)
	{
		...
		if (pkNewItem)
		{
			...
			{		
				...
				snprintf(szUpgradeAnnouncement, sizeof(szUpgradeAnnouncement), "<升级公告> 恭喜恭喜！玩家 [%s] 升级 [%s] 成功!", GetName(), pkNewItem->GetName()); 
...
#ifdef ENABLE_ANNOUNCEMENT_REFINE_SUCCES
			if (item->GetRefineLevel() >= ENABLE_ANNOUNCEMENT_REFINE_FAIL_MIN_LEVEL) 
			{		
				// ** If the item refined (given) after refining is greater than or equal to 7, then will announce the chat. ** //
				// ** I put it to check whether spam do not appear on all items, and +0,1,2,3,4,5,6 ** //
				char szUpgradeAnnouncement[QUERY_MAX_LEN];
				snprintf(szUpgradeAnnouncement, sizeof(szUpgradeAnnouncement), "<升级公告> 玩家 [%s] 升级 [%s] 失败!别灰心，勇者的路上总有挑战！", GetName(), item->GetName()); 
				BroadcastNotice(szUpgradeAnnouncement);
			}
#endif
		//PointChange(POINT_GOLD, -cost);
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
	...
	{
		if (NULL != DragonSoul_RefineWindow_GetOpener())
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("*E*AcA ? 상A?!1*는 3AAIAUA ?Aa 1 3o1AIU."));
		return false;
	}
	if (IsDead())
		return false;
	...
	ITEM_MANAGER::instance().RemoveItem(item);
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s   destroyed successfully."), item->GetName());
	return true;
}
	...
	if (item)
	{
		...
		if (item->AddToGround(GetMapIndex(), pos))
		{
			...
			if (false == LC_IsBrazil())
			{
				...
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("떨어진 아이템은 %d분 후 사라집니다."), 60/60);
			}
			else
			{
				item->StartDestroyEvent(60);
