...
void CHARACTER::Destroy()
{
	...
	if (m_pkRegen)	// this is what we need
	{
		if (m_pkDungeon)
		{
...
bool CHARACTER::Show(long lMapIndex, long x, long y, long z, bool bShowSpawnMotion/* = false */, bool bAggressive)
{
...
void CHARACTER::ApplyPoint(BYTE bApplyType, int iVal)
{
	switch (bApplyType)
	{
		...
		case APPLY_MELEE_MAGIC_ATTBONUS_PER:			// 86  +   +x%
#ifdef ELEMENT_NEW_BONUSES
...
#define MOUNT_SYSTEM_FIX_POLY
	...
	if (IsPolymorphed() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, "You cannot use a mount as long as you are polymorphed.");
		return; 
	}
#endif	
	if (GetMapIndex() == 113)
