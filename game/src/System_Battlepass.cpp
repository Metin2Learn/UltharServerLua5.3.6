...
void CHARACTER::DoMission(int index, long long count)
{
	...
	if (missions_bp[index].count == v_counts[index].count)
	{
		...
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("battle_pass_finished_mission"));
	...
	for (unsigned int i=0; i<v_counts.size(); ++i)
	{
...
void CHARACTER::ExternBattlePass()
{
	...
	Infos_BattlePass bp_infos = {0,0,0,0,0,0, 0, 0};
	...
	while (fgets(one_line, 256, fp))
	{
		...
		if ((v = strtok(NULL, delimit)))
			strlcpy(bp_infos.desc, v, sizeof(bp_infos.desc) - 1);
...
void CHARACTER::SendInfosBattlePass(int index)
{
	...
	ChatPacket(CHAT_TYPE_COMMAND, "info_missions_bp %d %d %d %s %s", index, v_counts[index].count, v_counts[index].status, rewards_bp[index].name, rewards_bp[index].desc);
}
...
void CHARACTER::FinalRewardBattlePass()
{
	...
	ChatPacket(CHAT_TYPE_INFO, LC_TEXT("battle_pass_finished"));
}