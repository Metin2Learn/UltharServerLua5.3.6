...
int DetermineFish(LPCHARACTER ch)
{
	...
	if (g_iUseLocale) // Áß±¹¿¡¼­´Â ±Ýµ¢¾î¸®, ±Ý¿­¼è, Àº¿­¼è ³ª¿ÀÁö ¾Ê°Ô ÇÔ
	{
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		...
		if ( rod->GetRefinedVnum()>0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,rod->GetValue(1))==1 )
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("³¬½Ã´ëÀÇ ¼ö·Ãµµ°¡ Áõ°¡ÇÏ¿´½À´Ï´Ù! (%d/%d)"),rod->GetSocket(0), rod->GetValue(2));
			if (rod->GetSocket(0) == rod->GetValue(2))
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("³¬½Ã´ë°¡ ÃÖ´ë ¼ö·Ãµµ¿¡ µµ´ÞÇÏ¿´½À´Ï´Ù."));
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾îºÎ¸¦ ÅëÇØ ´ÙÀ½ ·¹º§ÀÇ ³¬½Ã´ë·Î ¾÷±×·¹ÀÌµå ÇÒ ¼ö ÀÖ½À´Ï´Ù."));
			}
		}
	}
	// ¹Ì³¢¸¦ »«´Ù
...
void Take(fishing_event_info* info, LPCHARACTER ch)
{
	if (info->step == 1)	// °í±â°¡ °É¸° »óÅÂ¸é..
	{
		...
		switch (ret)
		{
			...
			case -3: // ³­ÀÌµµ ¶§¹®¿¡ ½ÇÆÐ
			case -1: // ½Ã°£ È®·ü ¶§¹®¿¡ ½ÇÆÐ
				//ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°í±â°¡ ¹Ì³¢¸¸ »©¸Ô°í Àì½Î°Ô µµ¸ÁÄ¨´Ï´Ù."));
				{
					int map_idx = ch->GetMapIndex();
					int prob_idx = GetProbIndexByMapIndex(map_idx);
			...
			case 0:
				...
				if (item_vnum)
				{
					...
					if (item)
					{
						...
						if (test_server)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ÀÌ¹ø¿¡ ÀâÀº ¹°°í±âÀÇ ±æÀÌ´Â %.2fcm"), item->GetSocket(0)/100.f);
						}

						if (quest::CQuestManager::instance().GetEventFlag("fishevent") > 0 && (info->fish_id == 5 || info->fish_id == 6))
						{
							// ÀÌº¥Æ® ÁßÀÌ¹Ç·Î ±â·ÏÇÑ´Ù.
							...
							if (info->fish_id == 5)
							{
								strlcpy(p.szBoard, LC_TEXT("³¬½ÃÀÌº¥Æ®¿ùÃ´ºØ¾î"), sizeof(p.szBoard));
							}
							else if (info->fish_id == 6)
							{
								strlcpy(p.szBoard, LC_TEXT("³¬½ÃÀÌº¥Æ®À×¾î"), sizeof(p.szBoard));
							}
...
void Simulation(int level, int count, int prob_idx, LPCHARACTER ch)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%d Á¾·ù %d ¸¶¸® ³¬À½"), fished.size(), total_count);
}
	...
	else
	{
		...
		switch (fish_info[idx].used_table[u_index])
		{
			...
			case USED_WATER_STONE:	// 2
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°í±â°¡ ÈçÀûµµ ¾øÀÌ »ç¶óÁý´Ï´Ù."));
				break;
			...
			case USED_SHELLFISH:	// 1
				...
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¹è ¼Ó¿¡¼­ Á¶°³°¡ ³ª¿Ô½À´Ï´Ù."));
				ch->AutoGiveItem(SHELLFISH_VNUM);
				break;

			case USED_EARTHWARM:	// 4
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¹è ¼Ó¿¡¼­ Áö··ÀÌ°¡ ³ª¿Ô½À´Ï´Ù."));
...
void Grill(LPCHARACTER ch, LPITEM item)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s¸¦ ±¸¿ü½À´Ï´Ù."), item->GetName());
	item->SetCount(0);
