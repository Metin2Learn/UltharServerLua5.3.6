...
ACMD(do_reload)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "RELOAD COMMANDS____________");
		ch->ChatPacket(CHAT_TYPE_INFO, "item_proto /reload pi");
		ch->ChatPacket(CHAT_TYPE_INFO, "mob_proto / reload pm");
		ch->ChatPacket(CHAT_TYPE_INFO, "object_proto / reload po");
		ch->ChatPacket(CHAT_TYPE_INFO, "skill_proto / reload pk");
		ch->ChatPacket(CHAT_TYPE_INFO, "refine_proto / reload pr");
		ch->ChatPacket(CHAT_TYPE_INFO, "item_attr / reload pa");
		ch->ChatPacket(CHAT_TYPE_INFO, "item_attr_rare / reload pe");
		ch->ChatPacket(CHAT_TYPE_INFO, "shop / reload ps");
		ch->ChatPacket(CHAT_TYPE_INFO, "banword / reload pb");
		ch->ChatPacket(CHAT_TYPE_INFO, "lands / reload pl");
		ch->ChatPacket(CHAT_TYPE_INFO, "object / reload pn");
		ch->ChatPacket(CHAT_TYPE_INFO, "quest / reload q");
		ch->ChatPacket(CHAT_TYPE_INFO, "gm_list / reload a");
		ch->ChatPacket(CHAT_TYPE_INFO, "offline_shop / reload shop");
		ch->ChatPacket(CHAT_TYPE_INFO, "drop / reload drop");
		ch->ChatPacket(CHAT_TYPE_INFO, "regen / reload regen");
		ch->ChatPacket(CHAT_TYPE_INFO, "group / reload group");
		return;
	}
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'c':
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, "Cube table reloaded.");
				Cube_init();
				bSendP2P = true;
				break;
			...
			case 'd':
				...
				sys_log(0, "Reloading drop information.");
				break;
#endif
			default:
				if (arg1[0] == 'p')
				{
					if (ch)
						ch->ChatPacket(CHAT_TYPE_INFO, "Reloading %c tables,", arg1[1]);
					db_clientdesc->DBPacket(HEADER_GD_RELOAD_PROTO, 0, &arg1[1], sizeof(char));
					bSendP2P = true;
				}
				break;
		}
	...
	else
	{
		...
		if (!strcmp(arg2, ""))
			strlcpy(p2p_packet.argument, "u yes", sizeof(p2p_packet.argument));
		else
	...
	if (ch && bSendP2P && !strcmp(arg2, "yes"))
	{
...
ACMD(do_aggregate_scope)
{
	...
	if (*arg1 && *arg2)
	{
		int maxDistance = 0;
		int maxMonsters = 0;
		str_to_number(maxDistance, arg1);
		str_to_number(maxMonsters, arg2);

		ch->AggregateMonsterScope(maxDistance, maxMonsters);
	}
