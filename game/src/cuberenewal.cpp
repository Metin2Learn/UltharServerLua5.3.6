...
void Cube_open(LPCHARACTER ch)
{
	...
	if (ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() || ch->isSashOpened(true) || ch->isSashOpened(false)
#if defined(__BL_67_ATTR__)
		|| ch->Is67AttrOpen()
#endif	
	)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_WINDOW_OPEN_FAILED"));
		return;
	}
...
void Cube_Make(LPCHARACTER ch, int index, int count_item, int index_item_improve)
{
	...
	if (!ch->IsCubeOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_NEED_OPEN_WINDOW"));
		return;
	}
	...
	for (TCubeResultList::const_iterator iter = resultList.begin(); resultList.end() != iter; ++iter)
	{
		if (index_value == index)
		{
			...
			if (materialInfo.gold != 0) {
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_NOT_ENOUGHT_YANG"));
					return;
				}
			}
			...
			if (material_check) {
				...
				if (iEmptyPos < 0)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_NOT_ENOUGHT_INVENTORY_FREE_SLOT"));
					return;
				}
				WORD objectSlot = -1;
#ifdef ENABLE_CUBE_ATTR_SOCKET
				...
				{
					for (unsigned int i=0; i<ch->missions_bp.size(); ++i)
					{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_UPGRADE_FAILED"));
					return;
				}
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("CR_NOT_ENOUGHT_MATERIAL"));
			}
		}
