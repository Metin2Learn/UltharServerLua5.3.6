...
struct FuncAggregateMonster
{
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 80) // 임시로 50% 확률로 적을 끌어온다
...
struct FuncAggregateMonsterScope
{
	LPCHARACTER m_ch;
	int m_maxDistance;
	int m_maxMonsters;
	...
	FuncAggregateMonsterScope(LPCHARACTER ch, int maxDistance, int maxMonsters)
		: m_ch(ch), m_maxDistance(maxDistance), m_maxMonsters(maxMonsters), m_currentMonsterCount(0) {}

	void operator()(LPENTITY ent)
	{
		if (m_currentMonsterCount >= m_maxMonsters)
			return;
		...
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 80) 
			{
				if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < m_maxDistance)
				{
					if (ch->CanBeginFight())
					{
						ch->BeginFight(m_ch);
...
void CHARACTER::AggregateMonsterScope( int maxDistance, int maxMonsters)
{
	LPSECTREE pSec = GetSectree();
	if (pSec)
	{
		FuncAggregateMonsterScope f(this,  maxDistance,  maxMonsters);
		pSec->ForEachAround(f);
	}
}
