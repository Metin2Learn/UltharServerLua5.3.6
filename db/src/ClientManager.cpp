...
void CClientManager::QUERY_RELOAD_PROTO(CPeer * pkPeer, const char * c_pData)
{
	//if (!InitializeTables())
	if (!InitializeTables(*c_pData))
	{
	...
	for (TPeerList::iterator i = m_peerList.begin(); i != m_peerList.end(); ++i)
	{
		...
		switch (LOWER(*c_pData))
		{
		case 'k':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TSkillTable)* m_vec_skillTable.size());
			tmp->Encode(c_pData, sizeof(char));
			tmp->EncodeWORD(m_vec_skillTable.size());
			tmp->Encode(&m_vec_skillTable[0], sizeof(TSkillTable)* m_vec_skillTable.size());
			break;

		case 'b':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TBanwordTable)* m_vec_banwordTable.size());
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'i':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TItemTable)* m_vec_itemTable.size());
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'm':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TMobTable)* m_vec_mobTable.size());
			tmp->Encode(c_pData, sizeof(char));
		...
		case 's':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TShopTable)* m_iShopTableSize);
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'r':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TRefineTable)* m_iRefineTableSize);
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'a':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TItemAttrTable)*m_vec_itemAttrTable.size());
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'e':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(TItemAttrTable)*m_vec_itemRareTable.size());
			tmp->Encode(c_pData, sizeof(char));
		...
		case 'l':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(WORD)+sizeof(building::TLand) * m_vec_kLandTable.size());
			tmp->Encode(c_pData, sizeof(char));
			tmp->EncodeWORD(sizeof(building::TLand));
			tmp->EncodeWORD(m_vec_kLandTable.size());
			if (m_vec_kLandTable.size())
				tmp->Encode(&m_vec_kLandTable[0], sizeof(building::TLand) * m_vec_kLandTable.size());
			break;
		case 'o':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(WORD)+sizeof(building::TObjectProto) * m_vec_kObjectProto.size());
			tmp->Encode(c_pData, sizeof(char));
			tmp->EncodeWORD(sizeof(building::TObjectProto));
			tmp->EncodeWORD(m_vec_kObjectProto.size());
			tmp->Encode(&m_vec_kObjectProto[0], sizeof(building::TObjectProto) * m_vec_kObjectProto.size());
			break;
		case 'n':
			tmp->EncodeHeader(HEADER_DG_RELOAD_PROTO, 0, sizeof(char)+sizeof(WORD)+sizeof(WORD)+sizeof(building::TObject) * m_map_pkObjectTable.size());
			tmp->Encode(c_pData, sizeof(char));
			tmp->EncodeWORD(sizeof(building::TObject));
			tmp->EncodeWORD(m_map_pkObjectTable.size());
			itertype(m_map_pkObjectTable) it = m_map_pkObjectTable.begin();
			while (it != m_map_pkObjectTable.end())
				tmp->Encode((it++)->second, sizeof(building::TObject));
			break;
		}
	}
}
